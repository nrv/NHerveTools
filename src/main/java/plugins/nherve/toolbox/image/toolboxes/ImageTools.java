...
public class ImageTools {
	...
	public static BufferedImage resizeAndCrop(BufferedImage original, int w, int h) {
		return resizeAndCrop(original, w, h, false);
	}

	public static BufferedImage resizeAndCrop(BufferedImage original, int w, int h, boolean keepOrigianlType) {
		BufferedImage resized = null;
		...
		Graphics2D g2 = (Graphics2D) resized.getGraphics();
		resizeAndCropAndDraw(original, g2, w, h);
		g2.dispose();

		return resized;
	}

	public static void resizeAndCropAndDraw(BufferedImage original, Graphics2D g2, int w, int h) {
		resizeAndCropAndDraw(original, g2, w, h, 0, 0);
	}

	public static void resizeAndCropAndDraw(BufferedImage original, Graphics2D g2, int w, int h, int x, int y) {
		double wr = (double) w / (double) original.getWidth();
		...
		int cx = (original.getWidth() - w) / 2;
		int cy = (original.getHeight() - h) / 2;

		g2.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_SPEED);
		g2.drawImage(original.getSubimage(cx, cy, w, h), x, y, null);
	}
